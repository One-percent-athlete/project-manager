...
const AddBoardForm: FC<Props> = props => {
    const { isVisible, toggleAddBoardForm, handleBoardSubmit, isSubmitting, boardData, updateBoardHandler } = props
  ...
  return <div className={`absolute w-80 z-[55] bg-gray-200 rounded-lg shadow ${isVisible ? "left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transition-transform duration-700" : "-translate-y-[150%] left-1/2 -translate-x-1/2"}`}>
    ...
    <div className="px-6 py-6 lg:px-8">
        ...
        <form onSubmit={handleBoardSubmit} className="space-y-6">
            <Input type="text" name="status" label="Board Status" placeholder="e.g To Do" required value={boardData.status} onChange={updateBoardHandler} />

            <button disabled={isSubmitting} type="submit" className="disabled:bg-gray-300 w-full text-white bg-blue-700 hover:bg-blue-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
