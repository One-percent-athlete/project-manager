...
const ProjectForm: FC<ProjectFormProps> = props => {
    const { isVisible, toggleProjectForm, handleSubmit, onChange } = props
    ...
    return <div className={`absolute w-80 z-[55] bg-white rounded-lg shadow ${isVisible ? "left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transition-transform duration-700" : "-translate-y-[150%] left-1/2 -translate-x-1/2"}`}>
        ...
        <div className="px-6 py-6 lg:px-8">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
                <Input label="Name" onChange={onChange} placeholder="Name" type="text" value={} name="name" required />
                <Input label="Description" onChange={onChange} placeholder="Description" type="text" value={} name="description" required />
            </form>
        </div>
    </div>
}
